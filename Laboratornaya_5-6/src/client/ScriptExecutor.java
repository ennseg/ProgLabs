...
public class ScriptExecutor {
    ...
    private Organization parseOrganization(String args) {
        ...
        if (fields.length != 12) {
            console.printError("Неверное количество аргументов для add. Ожидается 12, например add \"name\" x y annualTurnover \"fullName\" employeesCount type \"street\" \"zipCode\" townX townY \"townName\": " + args);
            return null;
        }
    ...
    private Organization parseUpdateOrganization(String args) {
        String[] fields = args.split("\\s+");
        if (fields.length != 13) {
            console.printError("Неверное количество аргументов для update. Ожидается 13: " + args);
            return null;
        }
